...
def dataset_detail(request, pk: int):
    ...
    if dataset.user is not None and request.user.is_authenticated and dataset.user != request.user:
        return Response(status=status.HTTP_403_FORBIDDEN)

    if dataset.user is not None and not request.user.is_authenticated:
        return Response(status=status.HTTP_403_FORBIDDEN)
...
def dataset_list(request):
    if request.user.is_authenticated:
        ...
        qs = UploadedDataset.objects.filter(user=None).order_by("-uploaded_at")
...
def enhance_view(request, pk: int):
    ...
    original_score = dataset.score
    original_metrics = dataset.metrics or {}
    ...
    dataset.enhanced_score = result["score"]
    dataset.enhanced_metrics = result["metrics"]
    dataset.original_score = dataset.original_score or original_score
    dataset.original_metrics = dataset.original_metrics or original_metrics
    dataset.save()
