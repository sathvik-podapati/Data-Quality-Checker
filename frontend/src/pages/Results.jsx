...
export default function Results() {
  ...
  const originalMetrics = data?.original_metrics || metrics
  const enhancedMetrics = data?.enhanced_metrics || metrics
  const score = data?.score ?? 0
  const originalScore = data?.original_score ?? score
  const enhancedScore = data?.enhanced_score ?? score
  ...
  const getCurrentMetrics = () => (viewMode === 'original' ? originalMetrics : enhancedMetrics)
  const getCurrentScore = () => (viewMode === 'original' ? originalScore : enhancedScore)

  const getCategoryValue = (key) => getCurrentMetrics()[key] ?? 0
      ...
      <div className="results-layout">
        <aside className="results-sidebar">
          <div className="score-header">
            <h2>Your Score</h2>
            <p className="score-value">{Math.round(getCurrentScore())}/100</p>
