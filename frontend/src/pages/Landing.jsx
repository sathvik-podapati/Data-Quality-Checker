...
export default function Landing() {
  ...
  const handleFileUpload = async (file) => {
    ...
    try {
      ...
      setProgress(100)
      const payload = res?.dataset ?? res
      const datasetId = payload?.id ?? payload?.dataset?.id
      if (!datasetId) {
        throw new Error('Upload completed but no dataset id was returned.')
      }
      localStorage.setItem(`dataset_${datasetId}`, JSON.stringify(payload))
